...
1. we tell R the *name* we want to assign our data frame. We're going to stick with old name of `ParticipationLevels`. 
2.  we tell R we want to *pipe in* the data from the existing data frame `ParticipationLevels` to tell it we're using that data in our function. 
3. we employ the function `mutate()` from the `dplyr` package to mutate (change) or data. 
4. we tell `mutate()` that we want to change the variable 'date' so that it is treated like a date using the function `as.Date()`. The thing we're changing is the variable `date`, so that's what `as.Date()` takes as an argument to its function. 
