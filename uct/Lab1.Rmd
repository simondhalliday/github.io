...
```{r}
ParticipationLevels <- read.csv("/Users/shalliday/Google Drive/website_backup/eco254/fall_2015/Labs/Lab1/more/labor_force_levels.csv")
```

`read.csv` instructs R to access the csv file and assign it to an object called `ParticipationLevels`. (I would also recommend the command `read_csv()` from the `readr` package. We shall use it later.)
