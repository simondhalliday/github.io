...
##Basics
The goal of this lab is to introduce you to R and RStudio, to revise the statistical concepts and to analyze real data and come to informed conclusions. 
...
##Reference point
Later, we'll talk about participation levels. To get a sense of what you can do with data, take a look at this graph at FRED: 
...
We'll leave it at that for now, but here you can see a lovely `dygraph` that can be generated using htmlwidgets (go to
[www.htmlwidgets.org](http://www.htmlwidgets.org/) to see a variety of other advanced options for what you can do when you become and R Champion). 
...
I would recommend that you save it in a workshop folder under a folder called Lab1 and then
create a backup of the file in that folder too. 
Open the file in MSExcel if you want to see what a `csv` looks like in Excel. 
Do not save any changes you make to it. 

Now, we are going to open the file in R using RStudio. 
First, we shall ensure that your *working directory* is set to the right place. 
Your working directory is the folder that contains your current project's files (this is even managed well in R Studio by creating 'Projects' that you sync with version control sites like GitHub).

First, let's check what the current working director is before we change it. Type this into the console: 
```{r working directory}
getwd()
```
Now, you might already be in the working directory you want to be in, but if you are not, then you may want to change your working directory, e.g.: 
```{r}
setwd("/Users/shalliday/Google Drive/simondhalliday.github.io/uct")
```
Do note that I'm writing filepaths for a Mac, so this may differ for you if you are working on a Windows, e.g. you'll need to include `C:\\` and use backslashes instead of forward slashes.
Notice, too, that if the working directory you set in this code chunk is different to the folder for this R Notebook (R Markdown file), then it'll be reset before the next code chunk. 
You can change the root directory for the Notebook as a whole in an initial setup code chunk using the `knitr` package option `root.dir`.

I have put the file `labor_force_levels.csv` in a folder called `more` in my `uct` folder. 
You could do something similar or choose an alternative name. 
...
`read.csv` instructs R to access the csv file and assign it to an object called `ParticipationLevels`. `read.csv` is from Base R and so you can use it whenever you want to, but it is not the fastest function to use for large .csv files. 

As an alternative, we could use `read_csv()` from the `readr` package. 
First we need to `require()` or `library()` the package `readr`:
```{r, warning = FALSE, message = FALSE}
library(readr)
ParticipationLevelsreadr <- read_csv("more/labor_force_levels.csv")
```

`ParticipationLevels` is a data table that we shall use to practise some basic analysis. 
You could use `ParticipationLevelsreadr` instead if you'd prefer. 

The data table contains the Bureau of Labor Statistics' counts for men and women participating in the labor force each period.
...
__objects__. 
Sometimes you load an object as we have done here, and
sometimes you create an object yourself as the result of a computation or
some analysis you have performed. 
Because you access the data from the web, this command (and the entire assignment) will work in a computer lab, in
...
numbers of men and women who participate in the labor force in the US. 
These days, the counts are published monthly. 
The BLS needs to understand whether someone participates in the labor force to come to judge employment in the US. 
You might remember from macro that the government measures 
employment only from those people participating in the labor force, that is, either working or looking for work. 
...
command (read as "fave stats""). What, you might ask, are social scientist's 
...
include the pipe as stories told by a 6-year-old:
...
```{r}
ParticipationLevels <- 
  ParticipationLevels %>% mutate(date = as.Date(date))
```
...
functions and a more complete sense of the language as the you progress. 
