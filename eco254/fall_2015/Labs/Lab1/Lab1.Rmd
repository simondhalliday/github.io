...
Before you start, be sure to load these packages, you could also use the command `library()`:
...
You can either type the command in manually or copy and paste it from this
document. So, as you can see, R is a good calculator. Later, we shall also
see that R can be used to graph functions and create a variety of interesting
graphics. 
...
the FRED database. You can access the `.csv` (comma separated value) file here:
...
save it in your Data Computing folder under a folder called Lab1 and then
create a backup of the file in that folder too. Open the file in MSExcel if
you want to see what a csv looks like in Excel. Do not save any changes you
make to it. 

Now, we are going to open the file in R using RStudio. Use the command
below, but be sure to change the filepath to the one where you have put the
`labor_force_levels.csv` file. 
...
This command instructs R to access the csv file and assign it to an object
called `ParticipationLevels`. ParticipationLevels is a data table that we
shall use to practise some basic analysis. 
