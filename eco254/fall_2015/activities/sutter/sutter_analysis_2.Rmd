---
title: "Sutter Experimental Data Analysis Part 2"
...
##Data Import
We now need to import the data. You should have edited and cleaned up the Excel file by now. You can alternatively download the csv of the file from Moodle here: [https://moodle.smith.edu/mod/resource/view.php?id=46861](https://moodle.smith.edu/mod/resource/view.php?id=46861).
...
```{r import data}
SutExp <- read.csv("/Users/shalliday/Google Drive/simondhalliday.github.io/eco254/fall_2015/activities/sutter/sutterexperiment.csv")
```

##Using rename to have consistent variable names
We want to have consistent naming protocols. So we shall quickly clean the data to make sure that all the variables are consistently labeled with lower case first letters to ensure we don't confuse them with data tables. To do this, we use the `rename()` command: 
...
- To find the averages we need to use `group_by` and `summarise`. - If you want to revise these commands, read Ch. 7 of *Data Computing* to be sure you understand how to use them and what they mean. 
- As you can see below, we want to `group_by()` with our Treatment variable (from Sutter's paper)
- We then need to use `summarise()` to create the *means* of the variable in which we're interested: R1 through R9 respectively. 
...
Please be sure to have gone through the process so far by Tuesday. 
...
Consider the following three sets of commands (Set 1, Set 2 and Set 3). Why do we get different values for the "average" for each treatment when we contrasts Set 1 with Sets 2 and 3? The importance of this question comes from the original Sutter data (check the *original* spreadsheet for the average values he reports). 
...
TreatAveSet1 <- 
  SutUniqAves %>% 
  group_by(treatment) %>%
  summarise(treatmean = mean(meanval))
TreatAveSet1
```
...
TreatAveSet2 <- 
  narrowAve %>% 
  group_by(treatment) %>% 
  summarise(meanval = mean(average))
TreatAveSet2
```
...
TreatAveSet3 <- 
  groupAves %>% 
  group_by(treatment) %>%
  summarise(treatmean = sum(r1, r2, r3, r4, r5, r6, r7, r8, r9)/9)
TreatAveSet3
```


The lesson here is that the *order* in which you do things matters for what you calculate. 

In Set 1, it used the data table `SutUniqAves` which had aggregated over individuals to find each individual's average contribution for *all* the 9 rounds. It then used that data to construct an average for the whole treatment. 

In Sets 2 and 3, on the other hand, the process was to aggregate over each round and then find the average over those rounds for the whole treatment. 

The point here is to emphasize that the point at which you choose to do an operation affects the average you will find. The problem, though, is that it is *not* clear from the paper (or from the supplementary files) where Sutter aggregated when he did his Mann-Whitney ranksum tests and the outcome we find depends on the point at which we aggregate, which we now demonstrate. 
